...
class Task{
    ...
    public :
    Task(string _videoPath,string _outputFilePath, string _animationPath , int _fps, int _maxFrames){
    ...
    void operator()() const
    {
        ...
        VideoFileInstance *contentVideo = new VideoFileInstance(1,imageSequence,videoPath.c_str(),outputFilePath.c_str());
...
void handle_post(http_request request)
{
    ...
    if(postParams.find("videoPath") != postParams.end()){
        ...
        videoPath = "/Users/apple/temp/Before_Vorator-2.mp4";
    }
    ...
    Task task(videoPath,outputFilePath,animationPath,fps,max_frames);
    std::thread thread(task);
    thread.detach();
    ...
    request.reply(status_codes::OK, "Hello Mr ");
}
...
int main()
{
    ...
    http_listener listener("http://localhost:8000/restdemo");

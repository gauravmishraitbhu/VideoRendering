...
int VideoFileInstance::cleanup(){
    ...
    int i=0;
    for( i=0; i < out_stream.format_ctx->nb_streams-1 ; i++){
        avcodec_close(out_stream.format_ctx->streams[i]->codec);
    }

    //close decoders
    for( i=0; i < ifmt_ctx->nb_streams-1 ; i++){
...
int VideoFileInstance::processVideoPacket(AVPacket *packet , int *frameEncodedCount, int *framesLeftInEncoder){
    ...
    *framesLeftInEncoder = frame_decoded;
    ...
    if(frame_decoded){
        ...
        if(nearestPercent != 0 && nearestPercent != lastReportedPercent && nearestPercent!=100){

            //100 percent will be reported explicitly by code after cleanup
...
int VideoFileInstance::convertToRGBFrame(AVFrame **yuvframe,AVFrame **rgbPictInfo) {
    ...
    if (imgConvertCtxYUVToRGB == NULL) {
        //init once
        imgConvertCtxYUVToRGB = sws_getContext(width, height, AV_PIX_FMT_YUV420P, width, height, AV_PIX_FMT_RGB24, SWS_LANCZOS, 0, 0, 0);
...
int VideoFileInstance::convertToYuvFrame (AVFrame **rgbFrame , AVFrame ** yuvFrame) {
    ...
    if(imgConvertCtxRGBToYUV == NULL) {
        imgConvertCtxRGBToYUV = sws_getContext(width, height, AV_PIX_FMT_RGB24, width, height, AV_PIX_FMT_YUV420P, SWS_LANCZOS, 0, 0, 0);
...
int VideoFileInstance::openFile() {
    ...
    jmethodID reportFileDurationCB = getMethodId(env, parentJavaThread, "reportInputFileDurationNativeCB", "(I)V");
    env->CallVoidMethod(parentJavaThread, reportFileDurationCB,videoDuration);
